(ns credit.operation
  "OperationActor -- one credit-granting operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (Credit-LLM) is sealed into a single node (:advise); its proposal is
  ALWAYS routed through the Credit Governor (:govern) and the rollout
  ...
  One graph run = one credit-granting operation (intake -> advise ->
  ...
  decision to a human operator (the licensed lender). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:loan/disburse` ALWAYS reaches this node when the governor is clean
  -- see `credit.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [credit.creditllm :as creditllm]
...
(defn build
  ...
  `credit.store/Store`).
  opts:
    :advisor      -- a `credit.creditllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (creditllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Credit-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (creditllm/-advise advisor store request)]
            {:proposal p :audit [(creditllm/trace request p)]})))

      ;; Credit Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
